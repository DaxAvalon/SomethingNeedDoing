...
function SND:InitItemCache()
  ...
  self.itemCache = {
    ...
    cachedItems = {},
    config = {
      maxPendingAge = 300,  -- 5 minutes before giving up
  ...
  -- Register GET_ITEM_INFO_RECEIVED event
  ...
  for _, itemID in ipairs(itemIDs) do
    ...
    if itemID and itemID > 0 then
      ...
      if self.itemCache.cachedItems[itemID] then
        -- Already cached, fire callback immediately if provided
        if callback then
        ...
        if callback and not pending.callbacks[callback] then
          ...
          pending.callbacks[callback] = true
        end
...
function SND:ProcessItemCacheBatch()
  ...
  -- Trigger UI refresh if items were cached
  if cached > 0 then
